import { definePlugin } from "vencord";
import { definePluginSettings } from "vencord";

const settings = definePluginSettings({
    stereochannel: {
        description: "Stereo Channel",
        type: OptionType.SELECT,
        options: [
            { label: "1.0 Mono", value: 1 },
            { label: "2.0 Stereo", value: 2 },
            { label: "7.1 Surround", value: 7.1, default: true },
        ],
    }
});

export default definePlugin({
    name: "EnableStereo",
    description: "Enables Stereo in Voice Calls. Note: Requires restart after every change. Noise suppression Krisp and Echo Cancellation must be disabled for it to work correctly.",
    authors: [
        {
            name: "Rattles",
            id: 1141097452741591140n
        }
    ],
    settings,
    patches: [
        {
            find: /channels:\d+/,
            replacement: {
                match: /channels:1,/,
                replace: `channels:1,params:{stereo:"1"},`,
                predicate: () => settings.store.stereochannel === 1
            }
        },
        {
            find: /channels:\d+/,
            replacement: {
                match: /channels:1,/,
                replace: `channels:2,params:{stereo:"2"},`,
                predicate: () => settings.store.stereochannel === 2
            }
        },
        {
            find: /channels:\d+/,
            replacement: {
                match: /channels:1,/,
                replace: `channels:8,params:{stereo:"7.1"},`,
                predicate: () => settings.store.stereochannel === 7.1
            }
        }
    ]
});
